% Created on : 05, January, 2022 by Danilo Braghini
...
clc 
%% Macros
global T1 T2 dmodel rho L1 L2 Lc A1 A2 c eta gamma_c H_pv kp ki kd ideal_filter kL Nd
T1=1;
T2=1;
...
%% initial conditions set up for w(k) method (SEM)
m=function_initialconditions_setup(pb,fv);
%% Inverse method via transfer matrix of SEM model
...
Vn = Css*V;
w_FEM = -1i*Lambda;
[w_FEM,ind] = sort(w_FEM,'ComparisonMethod','real');
Vn = Vn(:,ind); 
...
% [w_real_FEM_aux,w_imag_FEM_aux]= function_dipersion_FEM(ni,n1,n2,pb,M,C,K,gamma_c_ph,a,dmodel,kLv);
% % Detach frequency and damping factor for each bulk band and converting to kHz:
% for i=1:pb
%     w_real_FEM{i} = w_real_FEM_aux(i,:)/2/pi/1000;
%     w_imag_FEM{i} = w_imag_FEM_aux(i,:)/2/pi/1000;
% end
%% FRF of FEM model (local control only)     
...
% ne=np-1;
% P_FEM= function_FRF_FEM(F,wv,M,C,K,e,n1,n2,gamma_c_ph,ndof,ne,1);
% 
% %% FRF pressure by volume velocity plots
% figure
% %plot(fv/1000,20*log10(abs(P_SEM(1,:))),'r','MarkerSize',8,'LineWidth',1)
% hold on
% plot(fv/1000,20*log10(abs(P_FEM(1,:))),'r--','MarkerSize',8,'LineWidth',1)
% %plot(fv/1000,20*log10(abs(P_SEM(end,:))),'k','MarkerSize',8,'LineWidth',1)
% plot(fv/1000,20*log10(abs(P_FEM(end,:))),'k--','MarkerSize',8,'LineWidth',1)
...
% set(gcf, 'Color', 'w');
% grid on
%% Identifying via ss (FEM)
% single output
C=zeros(1,nx_cl);C(1,1)=1;
D=zeros(1,nw);
% multiple outputs
% C=eye(nx_cl);
% D=zeros(nx_cl,nw);
sys_siso=ss(Ass_cl,Bss_cl,C,D);
ts=0.0001;
t=0:ts:1;
h=impulse(sys_siso,t);
% df=fv(2)-fv(1);
% ts=1/(N*df);
% t=(0:(N-1))*ts;
%%
...
s=eig(A);
w_FEM_ident=-1i*s;
[w_FEM_ident,ind] = sort(w_FEM_ident,'ComparisonMethod','real');
%%
% Np=ncell;
% M=length(t);
% h=zeros(Ni*Np,M);
...
%   Caux(1,ii)=1;
%   for j=1:M
%       h(Ni*(ii-1)+1:Ni*ii,j)=Caux*A^(j-1)*B;
%    end
% end

%% Identifying eigenmodes via SEM 
% Nv=length(fv);
% H=P_SEM(1,:).';
...
% 
% %% Identifying eigenmodes via FEM 
% H=P_FEM(1,:).';
% H=[real(H(1));H(1:Nv);0;flipud(conj(H(1:Nv)))];
% N=length(H);
% df=fv(2)-fv(1);
...
%%
%-------------------------------------------------------------------------%
%    Unfolded Dispersion Diagram Plots by SEM and FEM with Eigenmodes  by FEM   
%                               and SEM                                   
%-------------------------------------------------------------------------%
% labelplot(1): wavenumber x real frequency
% labelplot(2): wavenumber x imaginary frequency
...
%zlim([-1 1])
hold on
scatter(imag(w_FEM/2/pi/1000),real(w_FEM/2/pi/1000),'k')
%scatter(imag(w_SEM_ident/2/pi/1000),real(w_SEM_ident/2/pi/1000),'*r')
scatter(imag(w_FEM_ident/2/pi/1000),real(w_FEM_ident/2/pi/1000),'*k')
 ylim([0 flim/1000])
%  xlim([0 1e-3]);
%function_unfolded_dispersion_diagrams(pb,kLv,w_real_FEM,w_imag_FEM,w_real_FEM,w_imag_FEM...
 %  ,'setplot',labelplot,'plotcolors',1);% ylim([0 1.35])
  xlim([-0.02 0.02]);
