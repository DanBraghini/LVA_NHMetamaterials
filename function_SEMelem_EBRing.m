...
% Flügge's theory (elementary rod + EB beam)

function D_elem_flexiblering2 = function_SEMelem_EBRing(R,A,I,rho,E,s0,omega)
% inputs
% R = raio [m]
...
%p3 = 0;
p2 = kappa^4-kB^4+2*kappa^2*kL^2;
%p2 = -E*I*E*A/R^2+E*I*rho*A*omega^2+E*A*rho*A*R^2*omega^2;
%p1 = 0;
p0 = -(kappa^4*kL^2+kappa^2*kB^4-kL^2*kB^4);
...
for cont2 = 1:1:length(rr)
    if imag(rr(cont2))< 0 
        r_prop_positivo(ar,1) = rr(cont2); ar=ar+1; 
        % números de onda que se propagam no sentido positivo
    else
        r_prop_negativo(br,1) = rr(cont2); br=br+1; 
