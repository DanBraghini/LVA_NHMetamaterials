...
%
% 29/08/2013
% modified on 08/02/2022 by Danilo Braghini to apply w(k) on stationary
% SEM element (uniform segment) for rings (closed curved beam) according to
% Love's theory for curved beams

function D_elem_flexiblering2 = function_SEM_elemEBRing(R,A,I,rho,E,s0,omega)
%D_elem_flexiblering2 = function_SEM_elemEBRing(R,A,I,rho,E,s0,omega)
% inputs
% R = raio [m]
% A  área da seção transversal [m^2]
% I  segundo momento de área [m^4]
...
im = 1i; % número imaginário
%
% Equação característica / Relação de dispersão
%kL=sqrt(rho*omega^2/E);
%kB=(rho*A*omega^2/E/I)^(1/4);
%kappa=1/R;
%p6 = 1;
p6 = -E*A*E*I*R^2;
p5 = 0;
...
rr = [gamma1;gamma2;gamma3;gamma4;gamma5;gamma6];
ar = 1; br = 1;

num=(2*rho*A*E*I*R^2*omega).*rr.^4+(2*rho*A*omega*((E*I+E*A*R^2))+...
    +2*rho*A*omega*(E*A-2*rho*A*R^2*omega)).*ones(6);
dem=(6*p6).*rr.^5+(4*p4).*rr.^3+(2*p2).*rr+p1.*ones(6);
dg_dw = -num./dem;
for i = 1:1:length(rr)
    gamma_i=rr(i);
    if imag(gamma_i)< 0 || (imag(gamma_i) == 0 && real(dg_dw(i)) > 0) 
        r_prop_positivo(ar,1) = rr(i); ar=ar+1; 
        % números de onda que se propagam no sentido positivo
    else
        r_prop_negativo(br,1) = rr(i); br=br+1; 
        % números de onda que se propagam no sentido negativo
    end
end
